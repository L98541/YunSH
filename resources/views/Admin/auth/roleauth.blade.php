                                        ...
                                        <div class="modal-body">
                                                <div>
                                                    <label for="ID">ID:</label>
                                                        <input  type="text"  class="form-control id " name="ID"  disabled placeholder="{{$user->name}}">
                                                </div><br>                                    
                                                <div>
                                                  
                                                    <label for="edit">管理员:</label>
                                                        <input  type="text"  class="form-control username useredityz usernameedit" reminder="用户名不能为空"  placeholder="{{$user->name}}" name="edit" name="edit1" disabled >
                                                </div><br>
                                                <div>
                                                    <label for="phone">权限列表:</label>
                                                        @foreach($auth as $auths)
                                                        <input type="checkbox" name="nid[]" id="model1" value="{{$auths->id}}"@if(in_array($auths->id,$nid)) checked @endif >
                                                        <label for="model1">{{$auths->name}}</label>
                                                        @endforeach
                                                </div><br>
                                        </div>
                                         
                                          <input type="hidden" value="{{$user->id}}" name="rid">
